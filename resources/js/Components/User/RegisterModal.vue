    ...
    <Modal @handleClose="$emit('handleClose')">
        ...
        <form class="max-w-[30rem]" @submit.prevent="submit" v-if="!isLoading">
            ...
            <div class="mt-5">
                ...
                :disabled="!register.designation || ([6, 7, 8, 9].includes(register.designation))"
                >
                    <option value="">select</option>
            ...
            <div class="mt-5">
                ...
                :disabled="!register.designation || ([1, 4, 5, 6, 7, 8, 9].includes(register.designation))">
                    <option value="">select</option>
                    <option 
                        :value="program.id" 
...
function getUser(){
    return axios.post('/user/get_profile', {
        id: props.user.id,
        edit: props.isEdit
    })
}

Promise.all([getAuthsAndDes(), getPrograms(), getInstitutes(), getUser()])
.then(res => {
    ...
    if(props.isEdit){
        register.auth = res[3].data.auth?parseInt(res[3].data.auth):null
        register.institute = res[3].data.instituteId?parseInt(res[3].data.instituteId):''
        register.program = res[3].data.programId?parseInt(res[3].data.programId):''
        register.designation = res[3].data.designation?parseInt(res[3].data.designation):''
    }
